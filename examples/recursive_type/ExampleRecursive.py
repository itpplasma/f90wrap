...
import _ExampleRecursive
import f90wrap.runtime
import logging
import numpy
import warnings

class Tree(f90wrap.runtime.FortranModule):
    """
    Module tree
    Defined at tree.fpp lines 5-21
    """
    @f90wrap.runtime.register_class("ExampleRecursive.node")
    class node(f90wrap.runtime.FortranDerivedType):
        ...
        def __init__(self, handle=None):
            ...
            if handle is not None:
                ...
                result = _ExampleRecursive.f90wrap_tree__treeallocate()
        ...
        def __del__(self):
            ...
            if getattr(self, '_alloc', False):
                _ExampleRecursive.f90wrap_tree__treedeallocate(root=self._handle)
        ...
        def left(self):
            ...
            """
            left_handle = _ExampleRecursive.f90wrap_tree__node__get__left(self._handle)
            if tuple(left_handle) in self._objs:
                left = self._objs[tuple(left_handle)]
            else:
                left = tree.node.from_handle(left_handle)
        ...
        def left(self, left):
            left = left._handle
            _ExampleRecursive.f90wrap_tree__node__set__left(self._handle, left)
        ...
        def right(self):
            ...
            """
            right_handle = _ExampleRecursive.f90wrap_tree__node__get__right(self._handle)
            if tuple(right_handle) in self._objs:
                right = self._objs[tuple(right_handle)]
            else:
                right = tree.node.from_handle(right_handle)
        ...
        def right(self, right):
            right = right._handle
            _ExampleRecursive.f90wrap_tree__node__set__right(self._handle, right)
